...
classDiagram
    ...
    class LinkStage {
      +metas: IndexVec&lt;ModuleIdx, LinkingMetadata&gt
    }
...
classDiagram
    ...
    class SymbolRefDb {
      +inner: IndexVec&lt;ModuleIdx, Option&lt;SymbolRefDbForModule&gt&gt
    ...
    class SymbolRefDbForModule {
        ...
        +flags: FxHashMap&lt;SymbolId, SymbolRefFlags&gt
        +classic_data: IndexVec&lt;SymbolId, SymbolRefDataClassic&gt
    ...
    class SymbolRefDataClassic {
        +namespace_alias: Option&lt;NamespaceAlias&gt
        +link: Option&lt;SymbolRef&gt
        +chunk_id: Option&lt;ChunkIdx&gt
    }
...
```mermaid
graph TD
    subgraph react_module_idx ["ModuleIdx for 'react' (react_module_idx)"]
        direction LR
        ReactMapKey["FxHashMapᐸCompactStr, IndexSetᐸSymbolRefᐳ"]
    end

    EIBM -- "key: react_module_idx" --> ReactMapKey

    ReactMapKey -- "key: 'useState' (CompactStr)" --> UseStateSet["IndexSetᐸSymbolRefᐳ: {sym_useState_main}"]

    class EIBM map;
    class ReactMapKey map;
    class UseStateSet set;
```
...
First, a helper function (`advance_import_tracker`) performs a quick, non-recursive analysis of the direct importee. It checks for simple cases and returns an initial status:
...
This initial status is often just the beginning. The real challenge is to trace the _entire_ re-export chain. The `mathc_import_with_export` function performs a recursive depth-first search to follow the symbol through any number of re-exports. The context for this search contains a stack to detect and handle import cycles, returning `MatchImportKind::Cycle` if one is found.

#### Phase 3: post-processing and finalization

After the main recursive search completes, two final checks are performed:

- **Ambiguity resolution**: the search might have collected several potentially ambiguous paths for a symbol (e.g., from multiple `export *` statements). Rolldown now checks if all these paths ultimately resovle to the **same** root symbol using the DSU `find` operation. If they lead to different original definitions, the impor is marked as truly ambiguous.

- **Shimming missing exports**: If the search results in `NoMatch`, Rolldown checks for a user-configured _shim_ to provide a fallback implementation. This is often used to ensure compatibility with older code that might rely on non-standard behavior.

Finally, the `MatchImportKind` result from this entire process is used to `link` the symbols in the `symbol_db`, creating the definitive connection.
