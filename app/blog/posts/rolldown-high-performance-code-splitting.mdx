...
This section focuses on the core logic of the chunk generation stage. Since the full generation process involves many moving parts -- such as handling namespaces, chunk linking, and wrappers -- we will narrow our focus to the __chunk generation algorithm__ itself.
...
The final step is to perform some optimizations and processing on the generated chunks. This includes merging external import namespaces, sorting modules and chunks, handling all entry level external modules.

## Merge external import namespaces at chunk level

The [external namespace](abbr:ExternalNamespace) was collected during the link stage, you can read my [previous article](https://www.atriiy.dev/blog/rolldown-link-stage-symbol-linking-resolution#match-imports-with-exports) to learn more about it. During the link stage, collect namespaces import symbols grouped by external modules. Now chunks are ready, we can start merging and linking the symbols at chunk level.

The process of merging is pretty straightforward. First, Rolldown will re-grouping the symbols based on the chunks they belongs to. In one chunk, multiple modules might import the same external namespace, and those modules might end up in the same chunk. Second, Rolldown confirms if current the modules are actually included in the bundle and sorts them by execution order. Finally, all same namespace symbols in the same chunk will be linked to the first symbols that's actually used.
...
All modules in every chunks will be sorted by execution order. After that, the chunks will also be sorted according to the chunks's kind. The ordering rules are as follows:
...
The entry-level external modules is an external module that is reachable from an entry module by following __only__ `export *` edges through normal modules. Here is an example:
...
Without optimization, Rolldown would need to generate complex interoperability code using the `__reExport` runtime helper, but it's not necessary. Therefore, this function detects when an external module is re-exported through an unbroken `export *` chain all the way to an entry point. When this happens, Rolldown can simply output:

```javascript
import * from 'external';
```

But an edge case, if someone actually uses the namespace object, which means not just re-exports it like the code above, Rolldown cannot optimize it. Because all symbols must be available on that namespace object.
